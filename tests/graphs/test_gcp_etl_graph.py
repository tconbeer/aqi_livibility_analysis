...
from aqi_livibility_analysis.graphs import gcp_etl_graph, site_locations_etl_graph
...
@pytest.mark.filterwarnings("ignore:The loop argument:DeprecationWarning")
@pytest.mark.filterwarnings("ignore:coroutine 'noop' was never awaited:RuntimeWarning")
def test_site_etl_graph() -> None:
    site_locations_etl_graph.execute_in_process(
        config={
            "download_site_data": {"config": {"target_date": "2018-01-01"}},
            "transform_site_data": {"config": {"target_date": "2018-01-01"}},
            "import_gcs_paths_to_bq": {
                "config": {
                    "destination": f"{GCP_PROJECT}.test.test_site_etl_graph",
                    "load_job_config": {
                        "source_format": "PARQUET",
                        "write_disposition": "WRITE_TRUNCATE",
                    },
            ...
            "fs": gcs_file_manager.configured(
                {
                    "project": GCP_PROJECT,
                    "gcs_bucket": "aqi-raw-data",
                    "gcs_prefix": "test/test_site_etl_graph",
                }
            ),
            "bigquery": bigquery_resource.configured({"project": GCP_PROJECT}),
        },
